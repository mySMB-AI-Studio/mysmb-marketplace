 ...
 */
const pct_of_max: ComputedFunction = (args) => {
  const value = Number(args.value ?? 0);
  const items = Array.isArray(args.items) ? args.items : [];
  const field = String(args.field ?? 'value');
  if (items.length === 0) return 0;
  const max = Math.max(...items.map((i: Record<string, unknown>) => Number(i[field] ?? 0)));
  if (max === 0) return 0;
 ...
 */
const win_rate: ComputedFunction = (args) => {
  const items = Array.isArray(args.value) ? args.value : [];
  const closed = items.filter((i: Record<string, unknown>) => i['IsClosed'] === true);
  if (closed.length === 0) return 0;
  const won = closed.filter((i: Record<string, unknown>) => i['IsWon'] === true);
 ...
 */
const flatten_quota_attainment: ComputedFunction = (args) => {
  const records = Array.isArray(args.value) ? (args.value as Record<string, unknown>[]) : [];
  ...
  type RepEntry = { name: string; initials: string; total: number };
 ...
 */
const flatten_by_state: ComputedFunction = (args) => {
  const records = Array.isArray(args.value) ? (args.value as Record<string, unknown>[]) : [];
 ...
 */
const state_total: ComputedFunction = (args) => {
  const records = Array.isArray(args.value) ? (args.value as Record<string, unknown>[]) : [];
  const countField = String(args.countField ?? 'cnt');
...
const membership_state_insight: ComputedFunction = (args) => {
  ...
  if (named.length === 0 || total <= 0) return 'Not enough data yet to surface a membership insight.';
 ...
 */
const state_summary: ComputedFunction = (args) => {
  const records = Array.isArray(args.value) ? (args.value as Record<string, unknown>[]) : [];
  if (records.length === 0) return { subtitleText: '', insightText: '' };
  const stateField = String(args.stateField ?? 'MailingState');
  const countField = String(args.countField ?? 'cnt');
...
const case_priority_tone: ComputedFunction = (args) => {
...
const elements: PluginElementsModule = {
  slug: 'salesforce',
  functions: { probability_tone, account_type_tone, pct_of_max, win_rate, sort_by_key, cycle_sort, flatten_quota_attainment, flatten_by_state, state_total, state_summary, membership_state_insight, membership_growth_insight, category_rank_tone, deal_stage_tone, or_dash, list_url, case_priority_tone, format_date, state_abbr },
};
