 ...
 */
const filter_below: ComputedFunction = (args) => {
  const items = Array.isArray(args.value) ? (args.value as Record<string, unknown>[]) : [];
  const field = String(args.field ?? '');
  const below = Number(args.below);
  if (!field || !Number.isFinite(below)) return items;
