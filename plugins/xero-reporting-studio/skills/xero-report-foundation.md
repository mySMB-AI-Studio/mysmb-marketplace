...
Confirm the Xero organisation, period or as-at date, accounting basis, and presentation currency — but never limit a report to a single organisation: every report must let the user pick the client to view from the list of organisations this Xero connection can access (via `list_connections`), and re-render the same report and layout scoped to that organisation's data only. Pass the selected organisation as `xero_tenant_id` on every subsequent data-fetching call for the rest of the report's lifetime — every Xero tool accepts this parameter — and persist the selection via `MyHubReport.setInputs` alongside the report's other inputs so it survives a refresh. Do not ask for an output format: every report is a single self-contained HTML document saved through `artifact_save` with a `.html` filename.
