...
function asAgents(value: unknown): AgentLite[] {
  return Array.isArray(value) ? (value as AgentLite[]) : [];
}
 ...
 */
function activeMentionToken(draft: string): string | null {
 ...
 */
const mention_matches: ComputedFunction = (args) => {
  const draft = String(args.draft ?? '');
  const token = activeMentionToken(draft);
  if (token === null) return [];
  const q = token.toLowerCase();
  const out: Array<{ id: string; name: string; schemaName: string; environmentId: string }> = [];
 ...
 */
const apply_mention: ComputedFunction = (args) => {
  const draft = String(args.draft ?? '');
 ...
 */
function splitMentions(text: string, names: string[]): { prefix: string; body: string } {
  const n = text.length;
  let cursor = 0;
  let end = 0;
  for (;;) {
 ...
 */
const mention_prefix: ComputedFunction = (args) =>
  splitMentions(String(args.text ?? ''), agentNames(args.agents)).prefix;
 ...
 */
const message_body: ComputedFunction = (args) =>
  splitMentions(String(args.text ?? ''), agentNames(args.agents)).body;
 ...
 */
const env_options: ComputedFunction = (args) => {
  const envs = Array.isArray(args.environments) ? args.environments : [];
  return envs
    .map((e) => {
      const env = (e ?? {}) as { id?: unknown; name?: unknown };
