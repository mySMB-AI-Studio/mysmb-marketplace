...
- **Service Desk Queue** (`atlassian-service-desk-queue`) — open requests in one JSM service desk, agent-facing (shows everything visible to the connected admin/agent account, not a per-customer "my tickets" view — see the note at the top of this README on why that tile is deferred). Each row's tone reflects the most urgent SLA clock still running on that request via the new `atlassian_sla_tone`/`atlassian_sla_label` widget-elements (`atlassian_sla_tone`: destructive = breached, warning = under 15% of goal duration remaining or under 1h when goalDuration is missing, muted = comfortable or no SLA clock running). Clicking a row opens the real ticket in Jira via the request's own `_links.agent` URL, confirmed against a real live response.
