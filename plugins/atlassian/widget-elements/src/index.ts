...
const space_type_label: ComputedFunction = (args) => {
  const raw = typeof args.value === 'string' ? args.value.trim() : '';
 ...
 */
const space_type_tone: ComputedFunction = (args) => {
 ...
 */
const issue_url: ComputedFunction = () => 'https://home.atlassian.com';
 ...
 */
const status_tone: ComputedFunction = (args): AtlassianTone | 'info' | 'success' => {
  const key = typeof args.value === 'string' ? args.value.trim().toLowerCase() : '';
  if (key === 'new') return 'warning';
  if (key === 'indeterminate') return 'info';
 ...
 * tone bucket.
 */
function prioritySeverity(name: unknown): number {
  const v = typeof name === 'string' ? name.trim().toLowerCase() : '';
  if (v === 'highest') return 5;
  if (v === 'high') return 4;
...
interface SortableIssue {
  key?: string;
  fields?: { priority?: { name?: string }; created?: string };
}
 ...
 */
const sort_issues: ComputedFunction = (args) => {
  const arr = Array.isArray(args.value) ? [...(args.value as SortableIssue[])] : [];
  const keyStr = String(args.key ?? 'priority|desc');
  const [field, dir] = keyStr.split('|');

  const valueOf = (issue: SortableIssue): number => {
    if (field === 'created') {
...
function workloadStatusFor(count: number): { label: WorkloadStatusLabel; tone: WorkloadStatusTone } {
  if (count <= 0) return { label: 'Available', tone: 'muted' };
  if (count <= WORKLOAD_ON_TRACK_MAX) return { label: 'On track', tone: 'success' };
...
function workloadLoadPercent(count: number): number {
...
const workload_by_agent: ComputedFunction = (args): WorkloadRow[] => {
  ...
  const byAccount = new Map<
    string,
    { accountId: string; displayName: string; count: number; searchUrl: string }
  >();
