...
function rankCategoryTones(categories: string[]): Map<string, string> {
 ...
 *   }
 */
const catalog_rows: ComputedFunction = (args) => {
  ...
  const categories = realProducts.map(
    (product) => (product.product_category as { name?: string } | null)?.name ?? 'Uncategorized',
  );
 ...
 *   }
 */
const category_breakdown: ComputedFunction = (args) => {
 ...
 *   }
 */
const brand_breakdown: ComputedFunction = (args) => {
  const products = Array.isArray(args.products) ? (args.products as Record<string, unknown>[]) : [];
  const realProducts = products.filter((product) => product.source !== 'SYSTEM');

  const counts = new Map<string, number>();
  for (const product of realProducts) {
    const brand = (product.brand as { name?: string } | null)?.name ?? 'Unbranded';
