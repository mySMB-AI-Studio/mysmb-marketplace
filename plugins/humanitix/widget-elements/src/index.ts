...
function stableCategoryTone(category: string): string {
...
function eventSubtitle(event: AnyRecord): string {
...
function formatDayTime(iso: unknown, timeZone?: string): string {
  const ms = Date.parse(String(iso ?? ''));
  if (!Number.isFinite(ms)) return '—';
  const d = new Date(ms);
  const opts: Intl.DateTimeFormatOptions = timeZone ? { timeZone } : {};
...
function capacityInfo(event: AnyRecord, categoryTone: string): CapacityInfo {
  const total =
...
function locationIconLabel(event: AnyRecord): { icon: string; label: string } {
  const label = eventLocationLabel(event);
  if (label === '—') return { icon: 'Globe', label: 'Online' };
 ...
 */
const flatten_upcoming_events_rich: ComputedFunction = (args) => {
