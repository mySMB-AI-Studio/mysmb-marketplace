...
const TICKET_TYPE_TONES = ['success', 'warning', 'destructive'];
const flatten_live_checkin = (args) => {
    const raw = (args.value ?? {});
    ...
    const ticketTypes = Array.isArray(raw.ticketTypes) ? raw.ticketTypes : [];
    return [
        {
 ...
 */
const flatten_revenue_trend = (args) => {
    const raw = (args.value ?? {});
    const dailyTotals = Array.isArray(raw.dailyTotals) ? raw.dailyTotals : [];
    const dayLabels = Array.isArray(raw.dayLabels) ? raw.dayLabels : [];
    ...
    return [
        {
            ...
            vs_last_week_badge_tone: vsLastWeekPct === null || vsLastWeekPct === 0 ? 'muted' : vsLastWeekPct > 0 ? 'success' : 'destructive',
