...
function objectTypeToneFor(raw: string): string {
  const i = OBJECT_TYPE_ORDER.indexOf(raw.trim().toLowerCase());
  return i === -1 ? 'muted' : OBJECT_TYPE_CHART_TONES[i];
}

const notion_object_type_tone: ComputedFunction = (args) => objectTypeToneFor(String(args.value ?? ''));
 ...
 *   }
 */
const notion_content_type_breakdown: ComputedFunction = (args) => {
  const results = Array.isArray(args.results) ? (args.results as Record<string, unknown>[]) : [];
...
function recencyBucketFor(lastEditedTime: string, now: number): string {
...
function recencyToneFor(bucket: string): string {
 ...
 */
const notion_page_recency_tone: ComputedFunction = (args) => {
 ...
 *   }
 */
const notion_recency_breakdown: ComputedFunction = (args) => {
  const results = Array.isArray(args.results) ? (args.results as Record<string, unknown>[]) : [];
...
function stageToneFor(stage: string): string {
  const i = STAGE_ORDER.findIndex((s) => s.toLowerCase() === stage.trim().toLowerCase());
  return i === -1 ? 'muted' : CHART_TONES[i];
}

const notion_stage_tone: ComputedFunction = (args) => stageToneFor(String(args.value ?? ''));
 ...
 *   }
 */
const notion_project_stage_breakdown: ComputedFunction = (args) => {
