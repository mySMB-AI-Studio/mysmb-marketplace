 ...
 */
const flatten_connections_health: ComputedFunction = (args) => {
  const raw = Array.isArray(args.value) ? (args.value as Record<string, unknown>[]) : [];

  let success = 0;
  let running = 0;
  let failed = 0;
  let mostRecentLastUsed: number | null = null;

  for (const conn of raw) {
    const status = String(conn.status ?? '').toLowerCase();
    if (status === 'active') success++;
    else if (status === 'pending' || status === 'pre-init') running++;
    else if (status === 'invalid') failed++;
 ...
 */
const connections_by_status: ComputedFunction = (args) => {
  const raw = Array.isArray(args.value) ? (args.value as Record<string, unknown>[]) : [];
  ...
  return raw
    .filter((conn) => {
      const status = String(conn.status ?? '').toLowerCase();
      if (bucket === 'pending') return status === 'pending' || status === 'pre-init';
      return status === bucket;
    })
    .map((conn) => {
      const institution = conn.institution as Record<string, unknown> | undefined;
      const name =
        (typeof institution?.name === 'string' && institution.name) ||
 ...
 * Args: { value: string }
 */
const connection_status_tone: ComputedFunction = (args) => {
  const status = String(args.value ?? '').toLowerCase();
  if (status === 'active') return 'success';
  if (status === 'pending' || status === 'pre-init') return 'info';
