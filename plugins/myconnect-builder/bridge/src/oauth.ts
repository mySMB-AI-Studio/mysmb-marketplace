...
export class FileAuthProvider implements OAuthClientProvider {
  ...
  constructor(
    ...
    if (existsSync(filePath)) {
      try {
        this.data = JSON.parse(readFileSync(filePath, 'utf8')) as PersistedAuth;
      } catch {
        this.data = {};
      }
    }
  ...
  codeVerifier(): string {
    if (!this.data.codeVerifier) throw new Error('No code verifier persisted');
    return this.data.codeVerifier;
  }
}

/** One-shot localhost server that resolves with the ?code= of the OAuth redirect. */
export function waitForCallback(port: number, timeoutMs = 5 * 60 * 1000): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    server = createServer((req, res) => {
      ...
      else reject(new Error(`OAuth redirect returned error: ${err ?? 'no code'}`));
    });
