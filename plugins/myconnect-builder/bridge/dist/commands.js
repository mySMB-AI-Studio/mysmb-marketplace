...
export function parseCommand(body) {
    ...
    if (lower === 'approve plan')
        return { type: 'approve_plan' };
    if (lower === 'approve release')
        return { type: 'approve_release' };
    if (lower === 'hold')
        return { type: 'hold' };
    if (lower === 'resume')
        return { type: 'resume' };
...
export function stageFromLabels(labels) {
    for (const s of STAGES) {
        if (labels.includes(`${STAGE_LABEL_PREFIX}${s}`))
            return s;
    }
    return null;
}
/** Replace all mcb:* stage/flag labels with the given stage (+ optional flags), preserving foreign labels. */
export function withStageLabels(labels, stage, flags = {}) {
    const kept = labels.filter((l) => !l.startsWith(STAGE_LABEL_PREFIX));
    const next = [...kept];
    if (stage)
        next.push(`${STAGE_LABEL_PREFIX}${stage}`);
    if (flags.blocked)
        next.push(BLOCKED_LABEL);
    if (flags.hold)
        next.push(HOLD_LABEL);
    return next;
}
