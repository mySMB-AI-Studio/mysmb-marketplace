 ...
 *   }
 */
const join_job_companies: ComputedFunction = (args) => {
  ...
  for (const company of companies) {
    ...
    if (uuid && name) nameByUuid.set(uuid, name);
  }
 ...
 */
const invoicing_summary: ComputedFunction = (args) => {
  const jobs = Array.isArray(args.value) ? (args.value as Record<string, unknown>[]) : [];

  const fmtAmt = (n: number): string =>
    new Intl.NumberFormat('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(n);

  const groups: Record<string, number> = { Quote: 0, 'Work Order': 0, Completed: 0, Other: 0 };
  let hasOther = false;
