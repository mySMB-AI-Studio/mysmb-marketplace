---
name: quickbooks-inventory
description: QuickBooks Online Inventory family (Q30) as a live, validated report in QuickBooks styling. Use when the user asks for inventory valuation, stock on hand value, inventory status, open purchase orders or a stocktake worksheet.
---
...
`get_report_inventory_valuation_summary` — expect columns '' | SKU | Qty | Asset Value | Calc. Avg with a GrandTotal TOTAL row; `list_item` (Inventory); `list_account` (AccountSubType Inventory); `list_purchase_order`. Verify on first run.
...
| Inventory Status | Report = Inventory Status (on hand, reorder point, on PO) |
| Open Purchase Order List | Report = Open Purchase Order List |
| Stocktake Worksheet | Report = Stocktake Worksheet (print with Download PDF) |
| Inventory Valuation Detail, Open Purchase Order Detail | N/A — not Accounting API reports |
...
`fileName`: `quickbooks-inventory-valuation.html` · `tags`: ["quickbooks","inventory"]
...
2. Compare the headline figures with the library example (illustrative, from Enterprise AI Pty Ltd — recompute on the golden set): Layout per QBO documentation: Product/Service | SKU | Qty | Asset value | Calc. avg cost.
3. Compare the layout with the Q30 screenshots (row order, "Total for" rows, header block, footer, number format).
...
```json
{
  "inputs": [
    ...
    },
    {
      ...
      "type": "string",
      "maxLength": 300,
      "default": "{\"cents\":1,\"k\":0,\"zeros\":1,\"neg\":\"minus\",\"red\":0,\"hdr\":1,\"ftr\":1,\"style\":\"qbo\",\"dens\":\"100\",\"p\":\"custom\",\"a\":\"custom\",\"c\":\"none\",\"v\":\"valuation\",\"x\":\"\"}"
    }
  ],
  "bindings": [
    {
...
```js
QB.app({
  title: 'Inventory Valuation Summary', token: 'INVENTORY_VALUATION_SUMMARY', route: 'reportv2', primary: 'inventory_valuation', company: 'company_info', prefs: 'prefs',
  inputs: { persona: 'persona', display: 'display' },
  defaults: { persona: 'Bookkeeper', display: '{"cents":1,"k":0,"zeros":1,"neg":"minus","red":0,"hdr":1,"ftr":1,"style":"qbo","dens":"100","p":"custom","a":"custom","c":"none","v":"valuation","x":""}' },
  uses: {},
