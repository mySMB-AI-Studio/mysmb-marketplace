---
name: quickbooks-projects-overview
description: QuickBooks Online Projects overview (Q13) as a live, validated report in QuickBooks styling. Use when the user asks for the projects overview, how projects are going, project income and profit, or estimates vs actual income for projects.
---
...
`get_report_profit_and_loss` with `summarize_column_by` = Customers (one column per customer and project); `list_customer` — projects carry IsProject = true (older companies: Job = true). Verify on first run: the sample company had no projects.
...
| Member / view | How |
|---|---|
| Estimates vs actual income | Report = Estimates vs actuals by project |
| Project Profitability Summary | Report = Project Profitability Summary |
| Projects overview | Income and profit per project, margin per project chart |
...
`fileName`: `quickbooks-projects-overview.html` · `tags`: ["quickbooks","projects","dashboard"]
...
2. Compare the headline figures with the library example (illustrative, from Enterprise AI Pty Ltd — recompute on the golden set): Captured empty (Total Income A$0, no projects) — verify on a company with projects.
3. Compare the layout with the Q13 screenshots (row order, "Total for" rows, header block, footer, number format).
...
```json
{
  "inputs": [
    ...
    },
    {
      ...
      "type": "string",
      "maxLength": 300,
      "default": "{\"cents\":0,\"k\":0,\"zeros\":1,\"neg\":\"minus\",\"red\":0,\"hdr\":1,\"ftr\":1,\"style\":\"qbo\",\"dens\":\"100\",\"p\":\"this_fy_td\",\"a\":\"custom\",\"c\":\"none\",\"v\":\"overview\",\"x\":\"\"}"
    }
  ],
  "bindings": [
    {
