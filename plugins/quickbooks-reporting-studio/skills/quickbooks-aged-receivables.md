---
name: quickbooks-aged-receivables
description: QuickBooks Online Accounts receivable family (Q24) as a live, validated report in QuickBooks styling. Use when the user asks for aged receivables, A/R ageing (aging), who owes us, debtors, overdue invoices, collections report, open invoices or customer balances.
---

# Accounts receivable family (Q24)

Use when the user asks for aged receivables, A/R ageing (aging), who owes us, debtors, overdue invoices, collections report, open invoices or customer balances. Load `quickbooks-report-foundation` first and follow its *Build a report* steps with the blocks below. This skill needs the `quickbooks-accounting` connector (`get_report_aged_receivables`, `get_report_aged_receivable_detail`, `list_invoice`, `get_report_balance_sheet`, `qbo_query`, `get_preferences`).
...
`get_report_aged_receivables` (no params) — expect columns Customer | Current | 1 - 30 | 31 - 60 | 61 - 90 | 91 and over | Total, one Data row per customer and a GrandTotal summary; note the ageing date in Header (Option report_date or EndPeriod).

## Date defaults

No date inputs: QuickBooks ages as of today. The header shows the date QuickBooks returned.

## Members

| Member / view | How |
|---|---|
| A/R ageing summary | Report = A/R ageing summary |
| A/R ageing detail | Report = A/R ageing detail (get_report_aged_receivable_detail) |
| Collections Report | Report = Collections report (overdue open invoices, oldest first) |
| Open Invoices | Report = Open invoices |
| Customer Balance Summary | Report = Customer balance summary |
| Customer Balance Detail, Invoice List, Invoices and Received Payments, Statement List, Terms List, Unbilled charges, Unbilled time | Wave 2 members |
| As-of date, ageing method, days per period, number of periods, min. days past due | N/A — the connector does not pass report_date / aging_period / num_periods / aging_method / past_due yet (connector change requested) |

## Validation checks (STEP 4 — shown in the banner)

- Row total = Σ bands for every customer
- Grand total = Σ customers (each band and total)
- Grand total = A/R on the balance sheet (today)
- Ageing detail open balance = summary total

## Save as

`fileName`: `quickbooks-aged-receivables.html` · `tags`: ["quickbooks","receivables","ageing","finance"]
...
2. Compare the headline figures with the library example (illustrative, from Enterprise AI Pty Ltd — recompute on the golden set): A/R as of 10 Sep 2026: Civica Pty Ltd A$39,105.00 (current); Total A$39,105.00.
3. Compare the layout with the Q24 screenshots (row order, "Total for" rows, header block, footer, number format).
...
```json
{
  ...
  "bindings": [
    ...
    {
      "id": "aged_receivable_detail",
      "tool": {
        "mcp": "quickbooks-accounting",
        "name": "get_report_aged_receivable_detail"
      },
      "params": {}
    },
    {
...
```js
var P = { kind: 'AR', title: 'A/R Ageing Summary', family: 'Accounts receivable', token: 'AR_AGING', party: 'Customer', ageing: 'aged_receivables', detail: 'aged_receivable_detail', docs: 'open_invoices', entity: 'Invoice', ref: 'CustomerRef', bsGroup: 'AR', bsRe: /^total accounts receivable/i, who: 'Who owes you', verb: 'owed to you',
  views: [['summary', 'A/R ageing summary'], ['detail', 'A/R ageing detail'], ['collections', 'Collections report'], ['open', 'Open invoices'], ['balance', 'Customer balance summary']] };
QB.app({
