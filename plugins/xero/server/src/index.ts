...
} from "./tools/get-profit-and-loss.js";
...
function loadEnvOrExit(): Record<(typeof REQUIRED_ENV)[number], string> {
  const missing: string[] = [];
  const out = {} as Record<(typeof REQUIRED_ENV)[number], string>;
  ...
  if (missing.length > 0) {
    process.stderr.write(
      `xero-mcp: missing required environment variable(s): ${missing.join(", ")}\n` +
...
interface ToolDef<I> {
  ...
  description: string;
  schema: z.ZodType<I>;
  handler: (client: XeroApiClient, input: I) => Promise<unknown>;
}
...
async function main() {
  ...
  const tools: ToolDef<any>[] = [
    def(
      "list_invoices",
      "List recent invoices for the connected Xero tenant. Optional status filter and limit.",
      listInvoicesInput,
      listInvoices,
    ),
    def(
      "get_contact",
      "Fetch a single Xero contact by id.",
      getContactInput,
      getContact,
    ),
    def(
      "create_invoice",
      "Create a draft invoice in Xero for the given contact and line items.",
      createInvoiceInput,
      createInvoice,
    ),
    def(
      "get_profit_and_loss",
      "Fetch the Xero Profit and Loss report for a date range.",
      getProfitAndLossInput,
      getProfitAndLoss,
    ),
  ];
...
main().catch((err) => {
  process.stderr.write(`xero-mcp: fatal: ${(err as Error).stack ?? err}\n`);
  process.exit(1);
});
